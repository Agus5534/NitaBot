...
public class TheBoyPanda implements EmbedHelper {
    public static void Panda(CommandEvent commandEvent) {
        int cantcartas = 0;
        int total = 0;
        try {
          cantcartas = (int) cartasconfig.get("CartaPanda."+commandEvent.getAuthor().getId(),cantcartas);
            ...
            eb.setDescription("Obtuviste: **El Paciente** <:Panda:871861308251856936>");
            eb.setThumbnail("https://cdn.discordapp.com/attachments/818995908779835393/835349972265205769/epico.png");
            eb.setImage("https://cdn.discordapp.com/attachments/818613273813319781/871923579388821564/1elpaciente.jpg");
            eb.setFooter(epicas1footer);
            eb.setColor(colorepico);
            commandEvent.getChannel().sendMessageEmbeds(eb.build()).queue();
            cantcartas++;
            cartasconfig.set("CartaPanda."+commandEvent.getAuthor().getId(),cantcartas);
            int desbloqueadas = 0;
             if(cantcartas == 1) {
